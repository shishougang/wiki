...
* Details of how SKB work[fn:1]
** the layout of the SKB data area
[[./Files/skb_layout.png]]
...
beginning of the data buffer. And the end pointer points to the end of
it. Note that all of the data area is considered tail room.
...
+ The *existence of paged data* in an SKB is indicated by =skb->data_len=
  ...
  =skb_is_nonlinear()= so that it the function you should use to test
  this.
...
copy data out from a packet into another buffer. This is not the case.
There are two helper routines available which make this pretty easy.
...
which is to be used *only* if the data you are interested in resides
...
use this routine to read and interpret protocol headers. The netfilter
layer uses this function heavily.
...
[fn:1] http://vger.kernel.org/~davem/skb_data.html
